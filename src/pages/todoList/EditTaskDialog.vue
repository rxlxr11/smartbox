  ...
  <view v-if="visible"
    ...
    <view
      ...
      <view
        ...
        <text class="text-[36rpx] font-700 text-[#101622]">编辑任务</text>
      </view>
      ...
      <!-- Footer -->
      <view
        class="flex justify-between gap-[24rpx] p-[32rpx] border-t-[2rpx] border-t-solid border-[#c4c6cf] bg-[#fcfcfd]">
        <view class="i-fa6-solid:trash-can text-[40rpx] text-[#ef4444] ml-[16rpx] self-center" @click="handleDelete">
        </view>
...
const emit = defineEmits<{
  ...
  (e: 'save', id: number, data: any): void;
...
const defaultFormData = () => ({
  ...
  unit: '',
  repeat_pattern: [0] as number[] // [0] means daily, [1,2,3] means Monday, Tuesday, Wednesday
})
...
watch(() => [props.visible, props.task], ([newVisible, newTask]) => {
  if (newVisible && newTask) {
    const t = newTask as TodoItem;
    let deadline = '';
    let time = '';
    if (t.deadline) {
    ...
    formData.value = {
      ...
      unit: t.unit || '',
      repeat_pattern: (t.repeat_pattern && Array.isArray(t.repeat_pattern)) ? t.repeat_pattern : [0]
    }
  }
})
...
const handleSave = () => {
  ...
  emit('save', props.task.id, payload)
}
</script>
